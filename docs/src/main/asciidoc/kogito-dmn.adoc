...
to power it up with DMN decision support.
...
A DMN model is an open standard for visual and semantic execution of declarative logic; DMN allows you to externalise decision logic into reusable pieces that can be easily used in declarative way. There are multiple ways of writing rules other than DMN, like: decision tables,
decision trees, rules, etc.
...
make use of the full capabilities offered by the Kogito platform, you can reference to the generic Kogito extension of Quarkus.
...
Create a new file `pricing.dmn` inside `src/main/resources/` directory of the generated project.
...
In the response, the `Base price` should be quoted for a `500` total amount, and that should also be visible in the response payload.
